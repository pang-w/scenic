...
function loadIndexAll(){
	...
	app.controller('iukissCtrl', function($scope,$http) {
		...
		$scope.searchArticlePrevious = function(){
			...
			var blogerName = $("#blogerName").val();
			support.ajax("page/blog/article", {"blogerName":blogerName,"pageIndex":(parseInt(page) - 1)}, function (result) {
		...
		var blogerName = $("#blogerName").val();
		support.ajax("page/blog/article", {"blogerName":blogerName}, function (result) {
		     $scope.articles = result.data;
		     $scope.$apply();
	    });
